...
class MaintenanceReportModel
{
    ...
    function getFilterCardCount($filters, $companyId) {
        $isActive = true;
        $sql = "SELECT
                    FORMAT(SUM(cmr.total_charge),0) AS total_charge,
                    FORMAT(COUNT(cmr.maintenance_report_id), 1) AS total_services,
                    FORMAT(ROUND(AVG(cmr.service_charge), 0), 1) AS avg_service_charge
                FROM
                    cms_maintenance_report cmr
                INNER JOIN
                    cms_maintenance_report_sub cmrs ON cmr.maintenance_report_id = cmrs.maintenance_id
                WHERE 1 = 1 ";
        ...
        if (!empty($filters['fromDate'])) {
            $sql .= "AND cmr.maintenance_date >= :fromDate ";
            $params[':fromDate'] = $filters['fromDate'];
        }

        if (!empty($filters['toDate'])) {
            $sql .= "AND cmr.maintenance_date <= :toDate ";
            $params[':toDate'] = $filters['toDate'];
        }

        if (!empty($filters['car'])) {
            $sql .= "AND cmr.car_id = :car ";
            $params[':car'] = $filters['car'];
        }

        if (!empty($filters['driver'])) {
            $sql .= "AND cmr.driver_id = :driver ";
            $params[':driver'] = $filters['driver'];
        }

        if (!empty($filters['spare'])) {
            $sql .= "AND cmrs.spare_parts = :spare ";
            $params[':spare'] = $filters['spare'];
        }

        if (!empty($filters['chargesFrom'])) {
            $sql .= "AND cmr.total_charge >= :chargesFrom ";
            $params[':chargesFrom'] = $filters['chargesFrom'];
        }

        if (!empty($filters['chargesTo'])) {
            $sql .= "AND cmr.total_charge <= :chargesTo ";
            $params[':chargesTo'] = $filters['chargesTo'];
        }

        $sql .= "AND cmr.company_id = :companyId ";
        $params[':companyId'] = $companyId;

        $sql .= "AND cmr.is_active = :isActive";
        $params[':isActive'] = $isActive;

        if (!empty($filters['orderBy'])) {
            // Ensure that the value is either ASC or DESC
            if ($filters['orderBy'] === "ASC" || $filters['orderBy'] === "DESC") {
                $sql .= " ORDER BY cmr.maintenance_date " . $filters['orderBy'];
            }
        }
    ...
    function getFilterFuelReport($filters, $companyId) {
        ...
        $sql = "SELECT 
                ...
                INNER JOIN cms_drivers cd ON cmr.driver_id = cd.id
                INNER JOIN cms_car cc ON cmr.car_id = cc.id
                INNER JOIN cms_maintenance_report_sub cmrs ON cmr.maintenance_report_id = cmrs.maintenance_id
                WHERE 1=1 ";
                //company_id = :companyId AND is_active = :isActive
        $params = [];
